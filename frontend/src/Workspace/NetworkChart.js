    ...
    useEffect(() => {
        const data = {
            ...
            edges: new DataSet(edges.map((edge, index) => {
                // Преобразуем weights в строку, если это не строка
                const weightsString = edge.weights.toString();
                const weightsArray = weightsString.split(',').map(Number); // Разделяем по запятой и преобразуем в числа
            ...
            {!dialogMode && hoveredNode && (
                ...
                >
                    ...
                    style={{
                        ...
                        top: (hoveredNode.position.y) - 350,   // Сдвиг для точности
                        pointerEvents: 'none',
